...
class DInterface(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'validation':
            self.valset = self.instancialize(
                file_path=self.kwargs['val_csv_path'], stage='validation')

        elif stage == 'fit' or stage is None:
            self.trainset = self.instancialize(
                file_path=self.kwargs['train_csv_path'], stage='train')
        ...
        if stage == 'test' or stage is None:
            ...
            self.testset = self.instancialize(
                file_path=self.kwargs['test_csv_path'], stage='test')
    ...
    def train_dataloader(self):
        return DataLoader(self.trainset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=True, collate_fn=lambda x: self.trainset.collate_fn(x))

    def val_dataloader(self):
        return DataLoader(self.valset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=False, collate_fn=lambda x: self.valset.collate_fn(x))

    def test_dataloader(self):
        return DataLoader(self.testset, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=False, collate_fn=lambda x: self.testset.collate_fn(x))
